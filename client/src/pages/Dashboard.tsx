...
import { LogOut, Heart, Search, Trash2 } from 'lucide-react'
...
export default function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  useEffect(() => {
    if (user?.fhirPatientId) {
      api.get(`/fhir/patient/${user.fhirPatientId}`)
  ...
  const givenName = patient?.name?.[0]?.given?.join(' ') || patient?.name?.[0]?.given?.[0] || ''
      ...
      <nav className="bg-white border-b shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold">MealCare</h1>
          </div>
            ...
            {Object.entries(grouped).map(([date, logs]: any) => {
              ...
              return (
                <div key={date}>
                  ...
                  {Object.entries(mealsByType).map(([mealType, mealLogs]: any) => (
                    <div key={mealType} className="mb-4">
                      ...
                      {mealLogs.map((log: any) => (
                        <div key={log.id} className="bg-white p-4 rounded-xl shadow-sm border mb-2">
                          ...
                          {log.items?.map((item: any) => (
                            <div key={item.id} className="text-sm mt-2 text-gray-600">
      ...
      {selectedFood && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-2xl w-96">
            ...
            <select value={mealType} onChange={(e) => setMealType(e.target.value)} className="w-full mb-3 border p-2 rounded">
            ...
            <button onClick={addMeal} className="w-full bg-emerald-600 text-white py-2 rounded">
              Add
            </button>
          </div>
        </div>
      ...
      {showLinkModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-2xl w-96">
            ...
            <button onClick={() => setShowLinkModal(false)} className="mt-4 w-full text-gray-500">
              Cancel
            </button>
          </div>
        </div>
